"""Detached history experiment supervisor with durable exit status."""
import argparse
...
def main():
    ...
    if a.output.exists(): raise FileExistsError('Fresh history output required')
    command = [sys.executable, '-u', str(code/'scripts/train_history_upper.py'),
        ...
        '--align-run', '/root/kinetalk_full_staged_20260917/repair12/align',
        '--centered-run', '/root/kinetalk_full_staged_20260917/repair12/centered_prior',
        '--output', str(a.output), '--epochs', '12']
    path = a.output.with_name(a.output.name+'_process.json')
    def write(value):
