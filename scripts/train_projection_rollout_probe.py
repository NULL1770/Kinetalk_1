"""Fixed18-epoch internal identity probe using one closed-loop motion loss.

Matches constant_teacher in data, initial weights, schedule and RNG draws.
Only its flow-velocity objective is replaced by the actual twelve-step final
motion MSE/residual_scale**2. Existing512-parameter projection alone trains.
"""
...
SCHEMA = "projection_rollout_probe_v1"
ARM = "constant_teacher_rollout"


def rollout_error(system, batch, controls, weight, noise):
    """Keep the full12-step autograd graph, including condition dependence."""
    ...
    error = ((prediction - q["motion"]) / system.residual_scale).square()
...
def main():
    ...
    recipe = {"schema": SCHEMA, "args": {**{key: str(value.resolve()) if isinstance(value, Path) else value for key, value in vars(args).items()}, "arm": ARM},
        ...
        "data_scope": data_scope, "trainable": ["local_projection.weight"], "loss": "observed_12step_rollout_motion_mse_divided_by_residual_scale_squared_only",
    ...
    for epoch in range(args.epochs):
        ...
        for ids in order.split(args.batch_size):
            ...
            error, _ = rollout_error(system, b, controls, w, noise.to(args.device))
            valid = observed(b["q"])
        ...
        record = {"arm": ARM, "epoch": epoch_done, "step": step, "samples_seen": len(features),
            "mean_rollout_loss": float(np.mean(losses)), "teacher_fraction": teacher_count / len(features),
            "rollout_mse_by_condition_source": {name: sums[0] / sums[1] if sums[1] else None for name, sums in source_errors.items()},
    ...
    print("COMPLETE fixed18epoch constant-teacher closed-loop-loss probe", flush=True)
