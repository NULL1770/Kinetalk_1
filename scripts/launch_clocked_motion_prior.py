...
def main():
    ...
    residual, prosody = mode.residual, mode.prosody
    args = base.parse_args()
    if args._supervise:
        return base.supervise(args._supervise)
    name = ('clocked_prosody_smoke' if args.smoke else 'clocked_prosody30') if prosody else (
    ...
    driver = args.code_root/('scripts/train_prosody_clocked_residual.py' if prosody else (
    ...
    if prosody:
        ...
        raise ValueError('--source-run is only valid with --prosody')
    if args.smoke:
