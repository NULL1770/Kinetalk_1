...
def main():
    ...
    residual, prosody, activity = mode.residual, mode.prosody, mode.activity
    args = base.parse_args()
    if args._supervise:
        return base.supervise(args._supervise)
    name = ('activity_condition_smoke' if args.smoke else 'activity_condition30') if activity else (
    ...
    driver = args.code_root/('scripts/train_activity_condition.py' if activity else (
    ...
    if prosody or activity:
        ...
        raise ValueError('--source-run is only valid with --prosody or --activity')
    if args.smoke:
