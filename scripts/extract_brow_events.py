"""Fixed fit-only brow excursion teacher; candidates are NOT semantic ground truth.
...
SCHEMA = "fixed_brow_excursion_teacher_v1"
...
PROTOCOL = {
    ...
    "overlap": "nonmatching overlapping candidates or fewer than four interevent quiet frames are composite; reject entire connected component",
    ...
    "deviation_from_proposal": "prominence plus quiet supports replaces unspecified joint-speed hysteresis; fixed before outcomes",
...
def _merge_candidates(candidates, smooth, source_id, run_index):
    ...
    for i, (a, b) in enumerate(bounds):
        for j, (c, d) in enumerate(bounds):
            if i < j and min(b, d)+4 > max(a, c): bad.update((i, j))
    events = []
...
def extract_clip(raw, valid, thresholds, source_id, fps=25.0):
    ...
    for run_index, (left, right) in enumerate(runs):
        ...
        # A short/edge candidate overlapping a complete event means the shared
        # five-dimensional shape is not cleanly represented either.
        for e in current[:]:
            overlaps = [c for c in candidates if not c["accepted"] and
                        min(e["end"], c["end"]) > max(e["start"], c["start"])]
            if overlaps:
                for c in candidates:
                    if c["accepted"] and min(e["end"], c["end"]) > max(e["start"], c["start"]):
                        c["accepted"] = False; c["reasons"].append("overlaps_unsupported_or_censored_motion")
                current.remove(e)
        for c in candidates:
            if not c["accepted"]: unsupported_mask[c["start"]:c["end"]+1] = True
        for e in current:
