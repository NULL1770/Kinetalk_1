...
最新进展：256片视觉语义提取、六臂30轮及固定300轮均已完成。300轮音频速度约参考1.75倍，显著降抖但时序条件只小幅胜静态，尚未成功；两套各四条视频已生成。见[新实测](VISUAL_SEMANTIC_PILOT_RESULTS_20260918.md)。当前没有进一步训练排队。前序r3阶段覆盖抬眉6.84%/压眉3.94%，早期85.1%仍无效；阶段标签只作诊断。

**稀疏眉部事件机制已实现，自然动态学习尚未通过。** 4段固定手工参数演示与32组一分钟延拓/控制检查完成，其他47通道固定且精确保留。32片教师修正过度否决规则后有20个完整候选，但可靠的已知结束等待为0，且多条候选与眨眼共现；正式先验未拟合，没有长训练排队。新增 v3 把左/右删失候选转为带 `known_mask` 的阶段状态，但仍未通过独立语义核验。见[本轮实测](SPARSE_BROW_EVENT_RESULTS_20260918.md)、[阶段状态结果](BROW_PHASE_STATE_V3_RESULTS_20260918.md)、[协议](SPARSE_BROW_EVENT_PROTOCOL_20260918.md)。
...
自然先验的验证已与音频活动预测分开推进。事件生成器能自动HOLD及完整起落，但不能把手工参数演示当学习成功。32片删失状态、眨眼/头姿共现及半独立像素运动诊断现已完成；完整事件/等待教师仍不充分。下一路线保留完整motion监督，以独立视觉时变语义条件作对照，不再强迫所有片段符合四阶段。原[设计审查](CONTROLLED_EVENT_PRIOR_DESIGN_REVIEW_20260918.md)保留作proposal历史。
