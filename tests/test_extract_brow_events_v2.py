...
from scripts import extract_brow_events_v2 as teacher
...
def test_unsupported_candidate_does_not_blanket_veto_complete_event():
    raw, valid = pulse()
    # Add an edge trend that yields a censored candidate overlapping the pulse.
    raw[:, 0] += np.linspace(.16, 0., len(raw))
    result, _ = teacher.extract_clip(raw, valid, THRESHOLDS, "overlap")
    assert any(c["left_censored"] or c["right_censored"] for c in result["candidates"])
    assert len(result["events"]) >= 1


def test_asynchronous_groups_can_remain_separate():
    raw, valid = pulse(1., "raise")
    down, _ = pulse(-1., "down")
    raw[:, :2] = down[:, :2]
    raw[:, :2] = np.roll(raw[:, :2], 5, axis=0)
    result, _ = teacher.extract_clip(raw, valid, THRESHOLDS, "async")
    assert result["summary"]["stage_counts"]["raise"]["candidates"] >= 1
    assert result["summary"]["stage_counts"]["down"]["candidates"] >= 1
